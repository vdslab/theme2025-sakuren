...
for search_word, word_counts in all_word_counts.items():
    ...
    min_y, max_y = int(np.min(np.where(mask_array < 128)[0])), int(np.max(np.where(mask_array < 128)[0]))
    min_x, max_x = int(np.min(np.where(mask_array < 128)[1])), int(np.max(np.where(mask_array < 128)[1]))
